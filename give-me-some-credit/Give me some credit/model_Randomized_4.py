...
#        print("%2d) %-*s %f" % (f + 1, 30, feat_labels[f], importances[indices[f]]))
    ...
    forest_rand_search = RandomizedSearchCV(rf, param_grid,
                                            ...
                                            n_jobs=9, random_state=42)


    #c.输出最佳模型并对测试数据进行预测
    #使用最优参数和training_new数据构建模型
    forest_rand_search.fit(x_train, y_train)
    print("the best parameter:", forest_rand_search.best_params_)
    print("the best score:", forest_rand_search.best_score_)

    #使用训练的模型来预测train_new数据
    predicted_probs_train = forest_rand_search.predict_proba(x_train)
    predicted_probs_train = [x[1] for  x in predicted_probs_train]
    computeAUC(y_train, predicted_probs_train)
    #使用训练的模型来预测test_new数据（validataion data）
    predicted_probs_test_new = forest_rand_search.predict_proba(x_test_new)
    predicted_probs_test_new = [x[1] for x in predicted_probs_test_new]
    computeAUC(y_test_new, predicted_probs_test_new)
    #使用该模型预测test data
    predicted_probs_test = forest_rand_search.predict_proba(x_test)
